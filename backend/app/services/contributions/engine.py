...
from .models import ContributionRecommendation, ContributionRequest, MinimumCheck, SleeveCalculation
...
def _check_minimum(product: ImplementationProduct, request: ContributionRequest) -> MinimumCheck:
    ...
    if additional and product.minimum_initial is not None:
        ...
        value, currency, kind = product.minimum_order, product.minimum_order_currency, "order"
    elif product.minimum_order_quantity is not None:
        return MinimumCheck(purchase_type=purchase, kind="quantity",
            applicable_minimum=product.minimum_order_quantity,
            minimum_currency=product.minimum_order_currency, status="verify_minimum",
            reason="quantity_minimum")

    common = dict(purchase_type=purchase, kind=kind, applicable_minimum=value, minimum_currency=currency)
    if value is None:
        return MinimumCheck(**common, status="verify_minimum", reason=reason)
    if currency != request.contribution_currency:
        return MinimumCheck(**common, status="verify_minimum", reason="currency_mismatch")
    needed = max(Decimal(0), value - request.contribution_amount)
    return MinimumCheck(**common, status="below_minimum" if needed else "ready",
...
def _recommend(request: ContributionRequest) -> ContributionRecommendation:
    ...
    common = dict(contribution_amount=request.contribution_amount,
        contribution_currency=request.contribution_currency, route_id=context.route_id,
        readiness=context.readiness, path=context.path, state=mapping.state,
        current_portfolio_value=total, post_contribution_portfolio_value=after)
    if request.contribution_amount == 0:
        return ContributionRecommendation(**common, action="no_action", recommended_amount=0,
            execution_status="not_applicable", reason="zero_contribution")
    if context.path == "short_term":
        return ContributionRecommendation(**common, action="no_action", recommended_amount=0,
            execution_status="not_applicable", reason="short_term_path", warnings=mapping.warnings)
    if not context.readiness.actionable_contribution_guidance_allowed:
        return ContributionRecommendation(**common, action="reserve", recommended_amount=request.contribution_amount,
            execution_status="preview", reason="foundation_reserve", warnings=mapping.warnings)
    ...
    for role in TIE_PRIORITY:
        ...
        calculations.append(SleeveCalculation(sleeve=role,
            target_percentage_points=item.target_percentage_points, current_value=current,
            current_percentage=current * 100 / total if total else None,
            target_value_after_contribution=target, deficit=target - current))
    candidates = [calc for calc in calculations if calc.deficit > 0 and mapped[calc.sleeve].actionable
                  and mapped[calc.sleeve].product.available_in_ph is not False]
    if not candidates:
        return ContributionRecommendation(**common, action="no_action", recommended_amount=0,
            execution_status="not_applicable", reason="no_eligible_deficit", calculations=tuple(calculations))
    ...
    if minimum.status == "verify_minimum":
        warnings.append("Verify the applicable minimum and eligibility in-app; no FX or price conversion is assumed.")
    ready = minimum.status == "ready"
    return ContributionRecommendation(**common, action="invest" if ready else "wait",
        recommended_amount=request.contribution_amount if ready else Decimal(0),
        execution_status=minimum.status, reason="greatest_deficit" if ready else minimum.status,
        selected=item, selected_calculation=selected, minimum=minimum,
        calculations=tuple(calculations), warnings=tuple(warnings))
