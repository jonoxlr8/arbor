...
def test_bitcoin_provider_parity_shared_reference_and_allocation(age, status):
    ...
    quote = price("btc_php", "3123456.123456789012", age=age).model_copy(update={"source": "coinranking"})
    ...
    assert [result.holdings[0].provider for result in results] == ["gcrypto", "coins_ph", "pdax"]
    ...
    if status == "unavailable":
        ...
        assert all(result.data_sources == ("coinranking",) for result in results)
