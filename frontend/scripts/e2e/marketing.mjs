...
const imagesReady=async()=>{
  for(const image of await page.locator('.marketing-site img').all()) {
    ...
    await page.waitForFunction(e=>e.complete&&e.naturalWidth>0,await image.elementHandle(),{timeout:20000}).catch(async()=>{
      throw new Error(`Public image failed to load: ${await image.getAttribute('alt')} (${await image.evaluate(e=>e.currentSrc)})`);
    });
  }
};
try {
