...
await withAuthenticatedBrowser(async ({page,reused})=>{
  ...
  try {
    ...
    stage='select investment';await page.locator('.catalogue-row[data-product="gcash_global_equity"]').click();
    ...
    await page.getByRole('button',{name:/^View ATRAM/}).click();await page.getByRole('button',{name:'Update current value',exact:true}).click();
    ...
    for(const width of [1440,390,320])for(const theme of ['Light','Dark']){
      ...
      layouts.push(`${width}/${theme}`);await page.getByRole('button',{name:'Close',exact:true}).click();
    }
    ...
    stage='save and contribution';await page.getByRole('button',{name:/^View ATRAM/}).click();await page.getByRole('button',{name:'Update current value',exact:true}).click();await saveValue('8000');
    ...
    await page.getByRole('button',{name:/^View ATRAM/}).click();await page.getByRole('button',{name:'Update current value',exact:true}).click();await saveValue('9000');
    stage='add units later';await page.getByRole('button',{name:/^View ATRAM/}).click();await page.getByRole('button',{name:/^Edit ATRAM/}).click();
    ...
    stage='clear';await page.getByRole('button',{name:/^View ATRAM/}).click();await page.getByRole('button',{name:'Update current value',exact:true}).click();
    ...
    await page.getByText(/Latest NAV updated/).waitFor();
    stage='cleanup';await page.getByRole('button',{name:/^View ATRAM/}).click();await page.getByRole('button',{name:/^Remove ATRAM/}).click();
