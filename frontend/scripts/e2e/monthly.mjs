...
await withAuthenticatedBrowser(async({page,reused})=>{
 ...
 try{
  ...
  let beforeHoldings;
  if(live){
   stage='fixture holding';const initial=page.waitForResponse(r=>new URL(r.url()).pathname==='/v2/portfolio');await go('portfolio');const first=await initial;assert.equal(first.headers()['x-arbor-monthly-fixture'],'isolated');assert.equal((await first.json()).holdings.length,0);
   ...
   const saved=page.waitForResponse(r=>new URL(r.url()).pathname==='/v2/portfolio'&&r.request().method()==='GET');await page.getByRole('button',{name:'Save holding record',exact:true}).click();beforeHoldings=(await(await saved).json()).holdings;
   await go('home');
  }
  ...
  await page.getByRole('button',{name:'Calculate scenario',exact:true}).click();const scenarioResponse=await scenario;assert.equal(scenarioResponse.status(),200);if(live)assert.equal((await scenarioResponse.json()).current_portfolio_value,'8000.00');
  await page.getByRole('heading',{name:'Choose options for this scenario',exact:true}).waitFor();for(const option of await page.getByRole('checkbox',{name:/Use /}).all())await option.check();
  ...
  for(const width of [1440,768,390,320])for(const theme of ['Light','Dark']){
   ...
   await go('portfolio/contribution');await region.getByRole('heading',{name:/check-in complete/}).waitFor();assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
  ...
  if(live){
   stage='holdings unchanged and cleanup';const reread=page.waitForResponse(r=>new URL(r.url()).pathname==='/v2/portfolio');await go('portfolio');const p=await(await reread).json();assert.deepEqual(p.holdings,beforeHoldings);assert.equal(p.history.length,1);
   await page.getByRole('button',{name:/^Remove ATRAM/}).click();const clean=page.waitForResponse(r=>new URL(r.url()).pathname==='/v2/portfolio'&&r.request().method()==='GET');await page.getByRole('button',{name:'Remove from Arbor',exact:true}).click();assert.equal((await(await clean).json()).holdings.length,0);
