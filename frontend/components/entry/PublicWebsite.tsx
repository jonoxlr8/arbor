...
function ProductImage({ name, width, height, alt, hero = false }: { name: string; width: number; height: number; alt: string; hero?: boolean }) {
  const mobile = ({ home: [390,844], portfolio: [390,950], ways: [390,950], holdings: [358,539], allocation: [358,201] } as Record<string,number[]>)[name];
  ...
  return <picture>{mobile && <source media="(max-width: 600px)" srcSet={`/product/completion/${name}-mobile.webp`} width={mobile[0]} height={mobile[1]}/>}
    <Image src={`/product/completion/${name}.webp`} width={width} height={height} alt={alt}
    ...
    <main id="public-content" tabIndex={-1}>
      <section className="m-hero" aria-labelledby="hero-title">
        ...
        <figure className="m-hero-product m-container">
          ...
          <figcaption>Actual Arbor interface · Illustrative data. Portfolio tracking is a product preview. Monthly check-ins are available in private beta.</figcaption>
        </figure>
      </section>
      ...
      <section id="customize-plan" className="m-record m-section" aria-labelledby="customize-title"><div className="m-container m-record-layout">
        <div><p className="m-eyebrow">Your plan, your choice · Product preview</p><h2 id="customize-title">Keep it simple.<br/>Or make it yours.</h2><p className="m-section-lead">Your core plan is complete as-is.</p><p>In the upcoming customization step, you can choose None, 5% or 10% for extra Technology or Bitcoin exposure. Nothing is added for you.</p><p>Arbor calculates the final mix from your choices. You review it, then confirm your plan.</p><p className="m-note">Technology may overlap with your global investments. Bitcoin can have much larger price swings. Both are optional.</p></div>
        <figure className="m-phone"><ProductImage name="customize" width={358} height={864} alt="Arbor Customize your plan preview with separate None, 5% and 10% choices for Technology and Bitcoin, both explicitly chosen by the user."/><figcaption>Optional customization · Included in Free</figcaption></figure>
      </div></section>
      ...
      <section id="monthly-contribution" className="m-container m-section m-monthly" aria-labelledby="monthly-title">
        <div className="m-monthly-copy"><p className="m-eyebrow">Invest this month · Arbor Plus</p><h2 id="monthly-title">A clear month.<br/>A longer view.</h2><p className="m-section-lead">See how your monthly contribution maps to the investments you chose.</p><p>Your plan, current portfolio values and chosen providers shape the calculation. The refined monthly preview groups exact amounts by provider and keeps amounts below a minimum visible.</p><p className="m-note">Contribution planning is available now with manual inputs. Saved implementation choices and this refined breakdown are part of the product preview. Holdings can supply current values when Live Portfolio is enabled.</p>
        ...
        <figure className="m-contribution-preview"><ProductImage name="contribution" width={358} height={291} alt="Arbor monthly provider summary showing an illustrative ₱7,680 assigned to VT and ₱2,320 to VGT through Gotrade, totaling ₱10,000. No trade has been placed."/><figcaption>Example calculation, not a buy list. You invest through your provider.</figcaption></figure>
      </section>
      ...
      <section id="pricing" className="m-pricing m-section" aria-labelledby="pricing-title"><div className="m-container"><div className="m-section-heading"><p className="m-eyebrow">A little clarity goes a long way</p><h2 id="pricing-title">Start with a plan.<br/>Grow with perspective.</h2><p className="m-section-lead">All beta users get Arbor Plus free.<br/>No card. No billing date. Just room to explore.</p></div>
        <div className="m-price-grid">
          <article className="m-price-free"><p className="m-eyebrow">Arbor Free · At launch</p><h3>Build your plan.<br/>See ways to invest.</h3><p className="m-price">₱0</p><ul><li>Investment profile &amp; readiness checks</li><li>Your choice of standardized plan &amp; targets</li><li>Optional Technology &amp; Bitcoin choices <small>Preview</small></li><li>Basic projections &amp; next steps</li><li>Ways to invest &amp; official provider links</li><li>10 Ask Arbor questions per month, when public quota support launches</li></ul><a className="m-text-link" href="#faq">Understand the plans <Arrow/></a></article>
          <article className="m-price-plus"><span className="m-beta-label">Private beta · Free for now</span><p className="m-eyebrow">Arbor Plus</p><h3>Track your portfolio.<br/>Stay aligned over time.</h3><p className="m-price">Free <span>during beta</span></p><ul><li>Everything in Free</li><li>Full Ask Arbor access, under fair use</li><li>Monthly contribution planning &amp; check-ins</li><li>Edit your profile or change your plan</li><li>Holdings, value history &amp; Plan Alignment <small>Preview</small></li></ul><StartLink/><p className="m-fine">Planned launch price: ₱399/month or ₱3,990/year.<br/>Display only. Payments are not available.</p></article>
