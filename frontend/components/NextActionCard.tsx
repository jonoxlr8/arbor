...
export function NextActionContent({action,onAction}:{action:NextAction;onAction:(destination:NextAction["destination"])=>void}) {
  return <section className="arbor-panel mb-5" aria-label="What should I do next?" data-action-key={action.key}>
    ...
    <button type="button" className="entry-primary mt-4 min-h-11 whitespace-normal" onClick={()=>onAction(action.destination)}>{action.button_label}</button>
  </section>;
}
export default function NextActionCard({userId,onAction}:{userId:string;onAction:(destination:NextAction["destination"])=>void}) {
