...
export function ApproachOptions({ options, selected, onSelect }: {options: Options; selected: string; onSelect: (value: Strategy | "short_term") => void}) {
  return <>
    ...
    <p className="mt-3 text-sm text-slate-600">Your self-assessment: {options.assessment.requested_strategy} volatility comfort. This describes your answers, not a plan selection.</p>
    ...
    <p className="mt-4 text-sm text-slate-500">You choose the model. Projections are hypothetical; these assumptions are not forecasts. Past performance does not guarantee future results. No satellite preferences are applied to these standard models.</p>
  </>;
}
export default function ApproachSelection({input,userId,existing,onComplete,onBack}: {input: ProfileV2Input; userId:string; existing?:boolean; onComplete:(plan:AccountPlan)=>void; onBack:()=>void}) {
  ...
  return <section className="arbor-panel mx-auto w-full max-w-xl"><button type="button" disabled={busy} onClick={onBack} className="entry-link mb-4 min-h-11">← Back</button>
    ...
    {options?<ApproachOptions options={options} selected={selected} onSelect={value=>{if(!busy)setSelected(value);}}/>:!error&&<p role="status">Loading approaches…</p>}
