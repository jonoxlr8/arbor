...
function Restriction({ result, historical }: { result: PreferenceApplication; historical?: boolean }) {
  return <>{result.reasons.map(reason => <p key={reason} className="mt-1 text-sm text-slate-600">
    {reason === "strategy_cap" ? (historical ? `The earlier assessment capped this request at ${result.strategy_cap_percentage_points}%.` : `Your strategy allows up to ${result.strategy_cap_percentage_points}% for this preference.`)
      : reason === "readiness_restricted" ? (historical ? "The earlier readiness check paused this allocation. Your request remains saved." : "Currently paused by your readiness check. Your request is still saved.")
      : "Saved only: long-term allocation does not apply to this short-term plan."}
  </p>)}</>;
}

export default function PreferencesV2({ value, historical = false }: { value: PlanV2; historical?: boolean }) {
  ...
  return <>
    <section className="arbor-panel">
      <h2 className="text-lg font-semibold text-slate-900">{historical ? "Saved historical requests" : "Your preferences"}</h2>
      ...
      <dl className="mt-3 divide-y divide-slate-200">{(["technology_tilt", "bitcoin"] as const).map(key => <div key={key} className="py-3">
        ...
        {result && <dd><Restriction result={result[key]} historical={historical} /></dd>}
      </div>)}</dl>
    </section>
    {value.plan.path === "long_term" ? <section className="arbor-panel">
      <h2 className="text-lg font-semibold text-slate-900">{historical ? "Earlier effective allocation" : "Effective target allocation"}</h2>
      <p className="mt-2 text-sm text-slate-600">{historical ? "The earlier assessment result, retained for reference. It is not your explicit standard-model selection." : value.plan.readiness.actionable_contribution_guidance_allowed ? "Your strategy with the preferences Arbor can currently apply." : "Strategy preview only. Extra technology and Bitcoin allocations are paused."}</p>
