...
function ProductRow({ item, amount, minimum, currency }: { item: MappedContribution; amount: string; minimum: MinimumCheck | null; currency: string }) {
  return <div className="min-w-0 border-t border-slate-200 py-4">
    ...
    <p className="break-words text-sm text-slate-700">Selected implementation option: {item.product.display_name}</p>
...
export default function ContributionResult({ result }: { result: Result }) {
  ...
  return <section aria-label="Contribution result" className="mt-6 min-w-0 rounded-2xl border border-slate-200 p-4">
    <h3 className="break-words text-xl font-semibold text-slate-900">{result.mode === "plan" ? `Your ${money(data.contribution_amount, data.contribution_currency)} scenario` : "Largest eligible target gap"}</h3>
    {reserve ? <><p className="mt-3 text-slate-700">Foundation First pauses investment allocations in this tool. This scenario accounts for the full contribution as reserve, without a purchase allocation.</p><p className="mt-2 break-all text-2xl font-semibold text-slate-900">Reserve · {money(data.contribution_amount, data.contribution_currency)}</p></>
      ...
      : result.mode === "plan" ? <>
        ...
        {nonzero(result.data.unallocated_amount) && <p className="mt-3 text-sm text-slate-600">{money(result.data.unallocated_amount, data.contribution_currency)} remains unallocated under this scenario’s minimum constraints.</p>}
    ...
    <p className="mt-4 text-xs leading-5 text-slate-500">A mathematical target-alignment scenario, not an instruction to buy or sell. You make your own investment decisions. Nothing has been bought or saved. Provider fees and eligibility may still apply.</p>
  </section>;
}
